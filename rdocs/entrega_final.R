...
print_quadro_resumo <- function(data, StoreMean, title="Medidas resumo
da receita total média das lojas nos anos de 1880 até 1889", label="quad:quadro_resumo1")
{
  var_name <- ensym(StoreMean)
  data <- data %>%
    summarize(Média = round(mean(!!sym(StoreMean)),2),
              `Desvio Padrão` = round(sd(!!sym(StoreMean)),2),
              `Variância` = round(var(!!sym(StoreMean)),2),
              `Mínimo` = round(min(!!sym(StoreMean)),2),
              `1o Quartil` = round(quantile(!!sym(StoreMean), probs =
                                              .25),2),
              `Mediana` = round(quantile(!!sym(StoreMean), probs = .5)
                                ,2),
              `3o Quartil` = round(quantile(!!sym(StoreMean), probs =
                                              .75),2),
              `Máximo` = round(max(!!sym(StoreMean)),2)) %>%
...
grafico_of<- ggplot(top_produtos_of) +
  aes(x = fct_reorder(NameProduct, Quantity, .desc=T), y = Quantity, label = Quantity) +
  geom_bar(stat = "identity", fill = "#A11D21", width = 0.7) +
  geom_text(
    position = position_dodge(width = .9),
    vjust = -0.5, #hjust = .5,
...
grafico_tt<- ggplot(top_produtos_tt) +
  aes(x = fct_reorder(NameProduct, Quantity, .desc=T), y = Quantity, label = Quantity) +
  geom_bar(stat = "identity", fill = "#A11D21", width = 0.7) +
  geom_text(
    position = position_dodge(width = .9),
...
grafico_fa<- ggplot(top_produtos_fa) +
  aes(x = fct_reorder(NameProduct, Quantity, .desc=T), y = Quantity, label = Quantity) +
  geom_bar(stat = "identity", fill = "#A11D21", width = 0.7) +
  geom_text(
    position = position_dodge(width = .9),
